<?php include("header.php"); ?>


<?php
// Creating DB Connection
$conn = mysqli_connect("localhost", "root", "", "register");

$id = $_GET['id'];
$register_query = "SELECT * FROM rgstr WHERE id='$id'";

// Execute
$register_query_run = mysqli_query($conn, $register_query);

if (mysqli_num_rows($register_query_run) > 0) {
    while ($row = mysqli_fetch_array($register_query_run)) {
?>

        <div class="container p-4">
            <div class="card">
                <div class="card-header bg-dark text-white text-center">
                    <h4> Update Deatils </h4>
                </div>
                <div class="card-body p-4" style="margin-left: 8%;">
                    <div class="row">
                        <div class="col-md-11">
                                
                            <form action="code.php" method="POST">
                                <!--  -->

                                <div class="mb-3">
                                    <input type="hidden" name="edit_id" class="form-control" value="<?php echo $row['id'] ?>">

                                    <label class="form-label">First Name</label>
                                    <input type="text" name="fname" class="form-control" value="<?php echo $row['fname'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Last Name</label>
                                    <input type="text" name="lname" class="form-control" value="<?php echo $row['lname'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Email address</label>
                                    <input type="email" name="email" class="form-control" value="<?php echo $row['email'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Password</label>
                                    <input type="password" name="password" class="form-control" value="<?php echo $row['password'] ?>">
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Contact</label>
                                    <input type="text" name="phone" class="form-control" value="<?php echo $row['phone'] ?>">
                                </div>

                                <a href="../index.php" class="btn btn-info"> Back</a>
                                <button type="submit" name="register_update_btn" class="btn btn-warning">Update Data</button>

                            </form>

                        </div>
                    </div>
                </div>
            </div>

        </div>
<?php
    }
} else {
    echo "No Data against this URL!";
}
?>

<?php include("footer.php"); ?>